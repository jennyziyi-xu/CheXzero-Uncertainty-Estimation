...
if __name__ == "__main__":
    ...
    # cxr_result_path = '../results/baseline/10_percent/cxr.csv'
    # bootstrap_result_path = '../results/baseline/10_percent/bootstrap.csv'
    ...
    threshold = np.percentile(test_pred, 10, axis=0)    # filter out 10% of lowest uncertainties. 
    ...
    for i in range(num_classes):
        ...
        test_true_i_copy = np.copy(test_true_i)
        test_true_i_bool = test_true_i.astype(bool)
        ...
        test_true_i_filtered = test_true_i_copy[test_true_i_bool].reshape(-1,1)
        print(test_true_i_filtered)
        test_pred_i_filterd = test_pred_i[test_true_i_bool].reshape(-1,1)
        # print(test_pred_i_filterd.shape)
        # test_true_i_filterd = test_true_i[filter_array].reshape(-1,1)
        
        cxr_result = eval.evaluate_by_class(test_pred_i_filterd, test_true_i_filtered, cxr_labels,i)
        print(cxr_result)
